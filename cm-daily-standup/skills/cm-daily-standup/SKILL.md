...
- **Git commits** — auto-discovers repos under your configured `reposRoot` (default `~/Code`)
  and pulls your commits (matched by your global `git config user.email`) in the window.
...
| `get_config` / `set_config` | Change the default `days` window or `reposRoot`. |
